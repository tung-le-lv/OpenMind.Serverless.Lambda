...
[Collection("OrderApi")]
public class AddOrderItemFunctionTests(OrderApiFixture fixture)
{
    private readonly AddOrderItemFunction _function = new(fixture.Mediator);
    ...
    [Fact]
    public async Task Handler_WhenValidItemSubmitted_ShouldAddItemToOrder()
    {
        var orderId = await CreateOrderAsync("cust-additem-1");

        var request = new APIGatewayProxyRequest
        {
            PathParameters = new Dictionary<string, string> { ["id"] = orderId },
            Body = JsonSerializer.Serialize(new { productId = "p2", productName = "Widget", quantity = 1, unitPrice = 5m }, JsonOptions)
        };
    ...
    public async Task Handler_WhenOrderNotFound_ShouldIndicateNotFound()
    {
        var request = new APIGatewayProxyRequest
        {
            ...
            Body = JsonSerializer.Serialize(new { productId = "p1", productName = "Widget", quantity = 1, unitPrice = 10m }, JsonOptions)
        };
