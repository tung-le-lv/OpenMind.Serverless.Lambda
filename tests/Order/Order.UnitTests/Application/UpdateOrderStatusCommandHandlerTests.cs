...
public class UpdateOrderStatusCommandHandlerTests
{
    ...
    public async Task Handle_WhenValidStatusTransition_ShouldUpdateOrderStatus()
    {
        ...
        var result = await _handler.Handle(new UpdateOrderStatusCommand(order.Id, OrderStatus.Confirmed), CancellationToken.None);
    ...
    public async Task Handle_WhenOrderDoesNotExist_ShouldReturnOrderNotFound()
    {
        ...
        var result = await _handler.Handle(new UpdateOrderStatusCommand("missing", OrderStatus.Confirmed), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenTransitionNotAllowed_ShouldRejectStatusChange()
    {
        var order = PendingOrder();
        _repo.Setup(r => r.GetByIdAsync(order.Id, It.IsAny<CancellationToken>())).ReturnsAsync(order);

        var result = await _handler.Handle(new UpdateOrderStatusCommand(order.Id, OrderStatus.Delivered), CancellationToken.None);
    ...
    public async Task Handle_WhenItemAdded_ShouldPublishEvents()
    {
        ...
        await _handler.Handle(new UpdateOrderStatusCommand(order.Id, OrderStatus.Confirmed), CancellationToken.None);
